...
// check if the cordinator id exists or not
  ...
  // register the students
    $query  = "INSERT INTO student(stu_name,stu_fname,image,addr,state,city,email,num,edu_pass,perc,year_pass,income,citizen,tenth,twelth,graduate,declar,pin,dob,categ)" ;
    $query .= " VALUES( '{$name}', '{$fname}','{$image}','{$address}','{$state}','{$city}','{$email}','{$num}','{$last_exam}','{$percent_mark}',{$year_passing},'{$annual_income}','{$citizen}','{$tenth}','{$twelth}','{$graduation}','{$decleration}','{$pincode}','{$dob}','{$category}' )";
